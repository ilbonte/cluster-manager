...
module.exports = class Instance {
  ...
  up(callback) {
    let executor = new Executor(this.instance);
    executor.up(callback);
  }
  poweroff(callback) {
    let executor = new Executor(this.instance);
    executor.poweroff(callback);
  }
  ...
  static getAll(sendCallback) {
    ...
    parallel({
      ...
      function (err, results) {
        ...
        console.log(JSON.stringify(mergedDocker));
...
function _mergeDocker(onlineData, savedData) {
  ...
  onlyOnline.forEach(id => {
    let [online] = onlineData.filter(item => item.Id === id);
    online.inspect = JSON.parse(JSON.stringify(online));
    online.runConfig={name:online.Names[0]};
    online.status = online.State;
    online.type = 'docker';
    online.name = online.Names[0];
    online.uid = online.Id;
    console.log('ONLYonline'+id,JSON.stringify(online));
    ret.push(online);
  });

  console.log('il ret',ret);
