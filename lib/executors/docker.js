...
module.exports = class DockerPlatform {
    ...
    build() {
        ...
        let onFinished = (err, output) => {
            if (err) {
                ...
                this.imageInstance.set('status', 'saved+built');
            }
  ...
  pause(callback){
    let container = docker.getContainer(this.image.containerId);
    console.log('pausing....');
    container.stop(function(err, data) {
      console.log('container stoppato');
      if (err) {
        callback(err);
      } else {
        callback(null, data);
  ...
  start(callback){
    ...
    container.start(function(err, data) {
      console.log('container startato');
      if (err) {
        callback(err);
      } else {
        callback(null, data);
