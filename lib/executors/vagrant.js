...
module.exports = class Vagrant {
  ...
  build() {
    ...
    fs.mkdir(imagePath, () => {
      ...
      machine.init(config.config.vm.box, config, () => {
        this.imageInstance.set('status', 'built');
        fs.readFile(imagePath+'/Vagrantfile', (err,text) => {

          this.imageInstance.set('config.build',text.toString());
  ...
  pause(callback){
    let machine = vagrant.create({
      cwd: this.image.inspect.cwd,
      env: process.env
    });
    machine.suspend(function (err, data) {
      if (err) {
        callback(err);
      } else {
        callback(null, data);
  ...
  start(callback){
    ...
    machine.resume(function (err, data) {
      if (err) {
        callback(err);
      } else {
        callback(null, data);
  ...
  poweroff(callback){
    ...
    machine.halt(function (err, data) {
      if (err) {
        callback(err);
      } else {
        callback(null, data);
  ...
  static builtImages(callback) {
    let builtImagesNames = [];
      ...
      each(folders, (folderName, intCallback) => {
        fs.readdir(basePath + folderName, (err, files) => {
          ...
          if (files.includes('Vagrantfile')) {
            builtImagesNames.push(folderName);
          }
          intCallback();
        });
      }, function (err) {
